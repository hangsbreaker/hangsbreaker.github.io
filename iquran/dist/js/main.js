...
_(".modaloverlay").forEach(el => el.addEventListener('click', event => {
...
_(".modalclose").forEach(el => el.addEventListener('click', event => {
...
function makeqlist(key = "") {
    let re = surah_list, table = '';
    for (i in re) {
        if (key == "" || re[i]['id'] == key || re[i]['surat_name'].toLowerCase().includes(key.toLowerCase()) || re[i]['surat_terjemahan'].toLowerCase().includes(key.toLowerCase()) || re[i]['surat_rename'].toLowerCase().includes(key.toLowerCase())) {
            ...
            <div class="col">
                ...
                <span class="type">${re[i]['type'] + ", " + re[i]['count_ayat'] + " Ayat"}</span>
            </div>
            <div class="col arabic" style="text-align:right;float:right;font-size:18px;">${re[i]['surat_text']}</div>
        </div>`;
        }
    }
...
async function carikata(qry = "") {
    ...
    if (qry != "") {
            ...
            for (r in arr) {
                ...
                if (ada == kw.length) {
                    ...
                    table += `<tr onclick="getsurah(${q},${n})">
                            <td class="ayah">
                                <div style="color:var(--color-title-text);font-weight:bold;margin-bottom:5px;">${surah_list[q]['surat_name'] + " (" + q + ":" + n + ")"}</div>
                                <div class="arabic" style="width:100%;text-align:right;font-size:23px;line-height:2.3;margin-bottom:10px;">
                                    ${arr[r]['teks_ayat']}
                                </div>
                                ${(transliterasi == 'true' ? `<span class="artr"><i>${arr[r]['bacaan']}</i></span>` : ``)}
                                ${(translate == 'true' ? `<span class="arid">${arr[r]['teks_terjemah']}</span>` : ``)}
                            </td>
                        </tr>`;
                }
            }
        ...
        for (i in re) {
            if (qry == "" || re[i]['id'] == qry || re[i]['surat_name'].toLowerCase().includes(qry.toLowerCase()) || re[i]['surat_terjemahan'].toLowerCase().includes(qry.toLowerCase()) || re[i]['surat_rename'].toLowerCase().includes(qry.toLowerCase())) {
                adalist = true;
            }
        }
        ...
        if (!adalist && table == '') {
            _("#list").innerHTML += '<div style="width:100%;text-align:center;padding:20px;margin-top:20px;color:var(--color-title-text);">&bull; &bull; &bull; &bull;</div>';
        } else {
            _("#list").innerHTML += '<table class="surah" style="overflow:hidden;border-bottom-left-radius:10px;border-bottom-right-radius:10px;">' + table + '</table>';
        }
    }
}
...
async function getsurah(surat = 1, nayah = "") {
    ...
    for (i in re) {
        ...
        ayah += `<tr id="n${re[i]['no_ayat']}">
            <td style="text-align:center;vertical-align:top;padding-top:15px;padding-left:15px;">
                <div class="star8" data-label="${re[i]['no_ayat']}"></div>
            ...
            <td class="ayah">
                ...
                ${(transliterasi == 'true' ? `<span class="artr"><i>${re[i]['bacaan']}</i></span>` : ``)}
                ${(translate == 'true' ? `<span class="arid">${re[i]['teks_terjemah'].replaceAll('<sup>', `<sup class="fnote" onclick="showfnote(${i})">`)}</span>` : ``)}
            </td>
        </tr>`;
    ...
    <tr>
        ...
        <td style="padding:10px;">
            ...
            <span class="type">${srh["type"] + ', ' + srh["count_ayat"] + ' Ayat'}</span>
        </td>
...
function gotoayah(i = 1) {
    ...
    window.scrollTo(window.scrollX, dims.top - 40);
    _("#modalgotosurah").modal("hide");
}

function showfnote(i = 0) {
    _("#modalwidget").modal("show");
    _("#widgetcontent").innerHTML = `<span class="widgettittle">Catatan</span>` + surah_data[i]["teks_fn"].replaceAll('</br>', '</br></br>');
}
...
function addmdlBookmark(mark = "") {
    ...
    if (book.length == 0) {
        _("#bookmarkfolder").innerHTML = '<div style="text-align:center;color:var(--color-text);">Belum ada bookmark</div>';
    } else {
        let listbook = '';
        for (b in book) {
...
function dataBookmark() {
    ...
    for (g in book) {
        ...
        for (b in mark) {
            ...
            listbook += `<div class="listbook" id="blist${mark[b]}" style="margin-left:25px;padding-bottom:2px;">
        <div class="delbook" style="float:right;" onclick="confdelBookmark('${book[g]}','${mark[b]}')">&times;</div>
        <div class="arabic" style="float:right;font-size:18px;">${surah_list[bm[0]]['surat_text']}</div>
        <div onclick="gotoBookmark('${mark[b]}')" style="width:100%;height:100%;">
            <span class="nmayah" style="display:block;">
                ${bm[0] + ') ' + surah_list[bm[0]]["surat_name"]}
            </span><small style="position:relative;top:-6px;">Ayat ${bm[1]}</small>
        </div>
    </div>`;
        }
        listbook += `</div>`;
    ...
    _("#bookmarkfolder").innerHTML = (listbook == '' ? '<div style="width:100%;text-align:center;padding:20px;margin-top:20px;color:var(--color-title-text);">&bull; &bull; &bull; &bull;</div>' : listbook);
}
...
function confdelBookmark(book = "", mark = "") {
    ...
    if (mark != "") {
        bm = mark.split("_");
        notif = `bookmark <b>${book}, ${surah_list[bm[0]]['surat_name']}</b>, Ayat ${bm[1]}?`;
    } else {
